...
<div>
	...
	<div class="flex rounded-xl bg-slate-100 p-1 mb-5">
		<button
			onclick={() => { mode = 'password'; magicSent = false; error = ''; }}
			class="flex-1 text-sm py-1.5 rounded-lg font-medium transition-all {mode === 'password' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}"
		>
			Mot de passe
		</button>
		<button
			onclick={() => { mode = 'magic'; error = ''; }}
			class="flex-1 text-sm py-1.5 rounded-lg font-medium transition-all {mode === 'magic' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}"
		>
			Magic link
		</button>
	</div>
	...
	{#if magicSent}
		<div class="text-center py-4">
			...
			<p class="text-sm text-slate-500 mt-1">Un lien de connexion a été envoyé à <strong>{email}</strong>.</p>
			<button onclick={() => { magicSent = false; }} class="text-sm text-blue-600 hover:underline mt-3">
				Renvoyer un lien
			</button>
		</div>
		...
		<form onsubmit={handlePasswordLogin} class="space-y-4">
			...
			<div>
				<div class="flex items-center justify-between mb-1.5">
					...
					<a href="/forgot-password" class="text-xs text-blue-600 hover:underline">Mot de passe oublié ?</a>
				</div>
				<input
					id="password"
					type="password"
	...
	{:else}
		<form onsubmit={handleMagicLink} class="space-y-4">
			<div>
				<label for="email-magic" class="block text-sm font-medium text-slate-700 mb-1.5">Email</label>
				<input
					id="email-magic"
					type="email"
					bind:value={email}
